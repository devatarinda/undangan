...
class ApiUndangan {
  final url = "http://172.20.10.2:80/api_undangan/getListUndangan.php";
  ...
  Future<Undangan?> cekUndangan(String email) async {
    final response = await http.get(Uri.parse(
        "http://172.20.10.2:80/api_undangan/cekUndangan.php?email=$email"));
    if (response.statusCode == 200) {
  ...
  Future<bool> updateKehadiran(Undangan undangan) async {
    final response = await http.get(Uri.parse(
        "http://172.20.10.2:80/api_undangan/updateKehadiran.php?undangan_id=${undangan.undanganId}"));
    if (response.statusCode == 200) {
      return true;
    } else {
      return false;
  ...
  Future<bool> resetKehadiran() async {
    final response = await http.get(
        Uri.parse("http://172.20.10.2:80/api_undangan/resetKehadiran.php"));
    if (response.statusCode == 200)
      return true;
    else
      return false;
